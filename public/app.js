...
function initChart() {
    ...
    historyChart = new Chart(ctx, {
            ...
            datasets: [
                {
                    ...
                    borderColor: '#00d2ff', backgroundColor: gradientWater,
                    borderWidth: 2, pointBackgroundColor: '#00d2ff',
                    pointRadius: 0, pointHoverRadius: 4,
                    fill: true, tension: 0.4, data: []
                },
                {
                    label: 'Soil Moisture (%)',
                    borderColor: '#00e676', backgroundColor: gradientSoil,
                    borderWidth: 2, pointBackgroundColor: '#00e676',
                    pointRadius: 0, pointHoverRadius: 4,
                    fill: true, tension: 0.4, data: []
                }
            ]
        ...
        options: {
            ...
            scales: {
                x: { grid: { color: 'rgba(255,255,255,0.05)', drawBorder: false }, ticks: { maxTicksLimit: 8 } },
...
function updateDashboard(data) {
    ...
    const labels    = reversedData.map(row => { const d = new Date(row.timestamp); return `${d.getHours().toString().padStart(2,'0')}:${d.getMinutes().toString().padStart(2,'0')}`; });
